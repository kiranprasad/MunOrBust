...
GLdouble curr=0, prev=0,movcord[3]={-200,-10,-200};
...
void initLights()
{
	...
	GLfloat whiteSpecularMaterial[] = {5.0,5.0,5.0},light_post0[]={0.0,0.0,10.0,1.0},whiteSpecularLight[] = {5.0, 5.0, 5.0},blackAmbientLight[] = {0.3, 0.3, 0.3},whiteDiffuseLight[] = {1.0, 1.0, 1.0},mShininess[] = {50},twoModel[]={GL_TRUE}; 
	glEnable (GL_DEPTH_TEST);
...
{   
	if(key=='+') movcord[1]--;
	if(key=='-') if(movcord[1] <0) movcord[1]++;
	if(key=='*')
	{
	...
	{
		x_r=0; y_r=0; z_r=0;
		viewer[0] = 1.0; viewer[1] = 0.0; viewer[2] = 0.0;
